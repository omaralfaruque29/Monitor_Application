...
def routeDefaultData(host_name, field_name, measurement):
	dict,query = getDefaultData(host_name, field_name, measurement)
...
def routeTimeIntervalData(host_name,field_name,from_date,to_date, measurement):
	dict,query = getTimeIntervalData(host_name,field_name,from_date,to_date, measurement)
        session['last_query'] = query
        print query
        return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'}
...
def routeForecastAlgo(algorithm, number_of_prediction, measurement):
	print session['last_query']
	if algorithm == 'arima':
        	dict = arima(host, port, user, password, dbname, measurement, session['last_query'],number_of_prediction)
                return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'}
        elif algorithm == 'ar':
                dict = ar(host, port, user, password, dbname, measurement, session['last_query'],number_of_prediction)
                return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'}
        elif algorithm == 'ma':
                dict = ma(host, port, user, password, dbname, measurement, session['last_query'],number_of_prediction)
                return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'}
        elif algorithm == 'holt':
                dict = holt(host, port, user, password, dbname, measurement, session['last_query'],number_of_prediction)
                return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'}
        elif algorithm == 'holtwinter':
                dict = holtwinter(host, port, user, password, dbname, measurement, session['last_query'],number_of_prediction)
                return jsonify({'results':dict}),200,{'Access-Control-Allow-Origin': '*'} 



def getDefaultData(host_name, field_name, measurement):
	client = DataFrameClient(host, port, user, password, dbname)
	if measurement == 'cpu':
		...
		query="SELECT mean("+field_name+") FROM "+measurement+" WHERE host ='"+host_name+"' AND time > now() - 7d GROUP BY time(1h) fill(0)"
	
	print query
	data = client.query(query)
	dataframe = data[measurement]
	dict={}
...
def getTimeIntervalData(host_name,field_name,from_date,to_date, measurement):
	...
	if measurement == 'cpu':
		...
		query="SELECT mean("+field_name+")/1000000 FROM kernel WHERE host ='"+host_name+"' AND (time >= '"+from_date+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	elif measurement == 'diskio':
		...
		query="SELECT mean("+field_name+") FROM "+measurement_name+" WHERE host ='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
...
def getFieldList(host, port, user, password, dbname, measurement_name):
	client = DataFrameClient(host, port, user, password, dbname)
	query ="show field keys from "+measurement_name+""
	print query
	data = client.query(query)
	dataframe = data[measurement_name]
	field_list = []
	for x in dataframe:
...
def arima(host, port, user, password, dbname, measurement_name, query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	print query
	data = client.query(query)
	dataframe = data[measurement_name]
...
def ar(host, port, user, password, dbname, measurement_name, query,number_of_prediction):
...
def ma(host, port, user, password, dbname, measurement_name, query,number_of_prediction):
...
def holt(host, port, user, password, dbname, measurement_name, query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	print query
	data = client.query(query)
	dataframe = data[measurement_name]
...
def holtwinter(host, port, user, password, dbname, measurement_name, query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	print query
	data = client.query(query)
	dataframe = data[measurement_name]
